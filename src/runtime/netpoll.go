...
// poll_runtime_isPollServerDescriptor return true if fd is the
// descriptor being used.
// This is useful because some OS (like aix) can have more than one fd
// inside netpoll implementation
func poll_runtime_isPollServerDescriptor(fd uintptr) bool {
	fds := netpolldescriptor()
	if GOOS != "aix" {
		return fd == fds
	} else {
		// Aix have a pipe on its netpoll implementation
		// Therefore, two fd are used.
		// There are returned by netpolldescriptor using a mask
